...
import logging
from .model_equations import LP_mistral_7b_A6000_sglang_extend_flashinfer as prefill_time
# from preble.benchmarks.exp_configs.model_equations import LP_Llama3_70B_H100_sglang_extend_flashinfer as prefill_time
...
class GlobalSchedulerWithTime:
    ...
    def runtime_selector_my(
        ...
        *args, **kwargs,
        ...
        with self.lock:
            split_nodes = {}
            leaf_node = self.cache.insert(key=tuple(input_ids), split_nodes=split_nodes)
            ...
            if leaf_node.num_tokens >= leaf_node.context_so_far: 
                runtime_idx_selected_result =  f"leaf_node.num_tokens {leaf_node.num_tokens} >= leaf_node.context_so_far {leaf_node.context_so_far}"
                gpu_selected = self.get_parent_gpu_allocation(leaf_node)
                runtime_id = int(np.argmin([self.per_gpu_load[gid] for gid in gpu_selected]))        
            else:
                runtime_idx_selected_result =  f"leaf_node.num_tokens {leaf_node.num_tokens} < leaf_node.context_so_far {leaf_node.context_so_far}"
                runtime_id = int(np.argmin(self.per_gpu_load))    
            self.per_gpu_load[runtime_id] += 1
            self.counter += 1
            self.update_gpu_allocation_for_parent(leaf_node, {runtime_id}) # Updated gpu allocations up till parent
            self.cache.update_allocated_size(leaf_node, runtime_id) # Update ref counters
        ...
        metrics = {
                ...
                "runtime_idx_selected_result":runtime_idx_selected_result
            }
        self.metrics_dict.append(metrics)
        return runtime_id, metrics
    ...
    def runtime_selector(
        ...
        *args, **kwargs,
        ...
        with self.lock:
            ...
            self.handle_split_nodes_gpu_allocations(split_nodes, self.gpu_allocations) # copies split node gpu allocation
            self.handle_split_node_histogram(split_nodes)
            runtime_idx_selected_result = None
            ...
            if leaf_node.num_tokens < leaf_node.context_so_far: # check that gpu allocation exists for important node
                ...
                if len(gpu_selected) > 1:
                    runtime_idx_selected_result = f"leaf_node.num_tokens {leaf_node.num_tokens} < leaf_node.context_so_far {leaf_node.context_so_far}, len(gpu_selected) > 1,calculate_min_load_cost"                    
                    runtime_idx = self.calculate_min_load_cost(leaf_node, gpu_selected)
                else:
                    runtime_idx_selected_result = f"leaf_node.num_tokens {leaf_node.num_tokens} < leaf_node.context_so_far {leaf_node.context_so_far}, len(gpu_selected) = 1"
                    runtime_idx = list(gpu_selected)[0]
            elif runtime_id_with_highest_hit_rate is not None:
                runtime_idx_selected_result = "runtime_id_with_highest_hit_rate"
                runtime_idx = runtime_id_with_highest_hit_rate
            else:
                runtime_idx_selected_result = f"leaf_node.num_tokens {leaf_node.num_tokens} >= leaf_node.context_so_far {leaf_node.context_so_far}, select from all possible gpu"
        ...
        self.metrics_dict.append(metrics)
        return runtime_idx, metrics
    def finish_request_my(
        ...
        with self.lock:
            ...
            self.per_gpu_load[runtime_id] -= 1 # 后加的
