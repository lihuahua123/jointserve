...
async def generate_request_helper(obj: GenerateReqInput):
    ...
    if obj.lora_uid is not None:
        payload["lora_uid"] = obj.lora_uid
    print(payload,url)
    text = obj.text
...
def start_server(runtime_selection_policy="custom", runtime_urls="http://127.0.0.1:30000/generate", host='127.0.0.1', port=8000, model="/hy-tmp/"):
    """
    ...
    Args:
        ...
        model (str): The model name or path to be used by the server. Defaults to "/hy-tmp/".
...
def start_server_and_load_models(model_name="/hy-tmp/", devices=[0], host="127.0.0.1", port=8000):
    """
    Loads the specified model onto the given devices and starts the server.

    Args:
        model_name (str): The name or path of the model to be loaded. Defaults to "/hy-tmp/".
