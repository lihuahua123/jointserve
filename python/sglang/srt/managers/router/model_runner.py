...
class ModelRunner:
    def __init__(
        ...
        if self.lora_paths:
            ...
            self.lora_manager.load_loras_from_path(self.lora_paths)
    ...
    def forward_prefill(self, batch: Batch,lora_uids=None):
    ...
    @torch.inference_mode()
    def forward_extend(self, batch: Batch,lora_uids=None):
        input_metadata = InputMetadata.create(
            self,
    ...
    @torch.inference_mode()
    def forward_decode(self, batch: Batch,lora_uids=None):
        input_metadata = InputMetadata.create(
            self,
    ...
    def forward(self, batch: Batch, forward_mode: ForwardMode):
        ...
        if self.is_multimodal_model and forward_mode == ForwardMode.EXTEND:
            ...
            return self.forward_decode(batch,lora_uids)
        elif forward_mode == ForwardMode.EXTEND:
            return self.forward_extend(batch,lora_uids)
        elif forward_mode == ForwardMode.PREFILL:
            return self.forward_prefill(batch,lora_uids)
        else:
